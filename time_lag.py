"""compute time-based lag for com_tool or its variants in RQ1~2"""
import os
import pandas as pd
...
def compute_time_lag(dep:dict)->(int,int, int):
    try:
        gid = dep["GroupId"]
        aid = dep["ArtifactId"]
        ...
        original_lag = get_time_lag_of_a_dep(gid, aid, original_version)
        best_lag = get_time_lag_of_a_dep(gid, aid, best_version)
        return original_lag, best_lag, original_lag - best_lag
    except Exception as e:
        print(f"Fail to handle {gid}:{aid}, reason:{e}")
        return 0,0,0
...
def main():
    ...
    data_dir = os.path.join(DATA_DIR, 'naive_result')
    ...
    csv_path = os.path.join(DATA_DIR, 'naive_time_lag.csv')
    dataset_df = pd.DataFrame(columns=['repo_name', 'module_name', 'original_lag', 'best_lag', 'reduced_lag'])
    ...
    with tqdm(total=len(modules)) as pbar:
        for module in modules:
            ...
            json_path = os.path.join(data_dir, repo_name, module_name, 'version.json')
            if os.path.exists(json_path) == False:
                pbar.update(1)
                continue
            deps = get_deps_from_json(json_path)
            for dep in deps:
                o, b, r = compute_time_lag(dep)
                original_lag += o
                best_lag += b
                reduced_lag += r
            
            new_row = pd.DataFrame([[repo_name, module_name, original_lag, best_lag, reduced_lag]], \
                columns=['repo_name', 'module_name', 'original_lag', 'best_lag', 'reduced_lag'])
            numeric_columns = ['original_lag', 'best_lag', 'reduced_lag']
