"""evaluate the performance of Snyk
...
def main():
    """main function"""
    log_path = os.path.join(DATA_DIR, 'snyk_log.txt')
    # clear log
    ...
    csv_path = os.path.join(DATA_DIR, 'snyk_dataset.csv')
    # Create or load CSV file
    if os.path.exists(csv_path):
        dataset_df = pd.read_csv(csv_path)
    else:
        # Create a DataFrame with the header if the CSV does not exist
        dataset_df = pd.DataFrame(columns=['repo', 'module', 'snyk_compile_success', 'snyk_test_pass',
                                           'original_tech_lag', 'snyk_current_tech_lag', 'snyk_reduced_tech_lag', \
                                               'original_dep_count', 'snyk_current_dep_count', 'snyk_reduced_dep_count'])
    ...
    with tqdm(total=len(modules)) as pbar:
        for module in modules:
            ...
            row_value = evaluate_snyk(module)
...
def evaluate_snyk(module):
    """evaluate the performance of snyk
    Return:
        row_value: list which is a row in the dataset
    """
    repo_name, module_name = module[0], module[1]
    snyk_dir_path = os.path.join(DATA_DIR, 'snyk', repo_name, module_name)
    if not os.path.exists(snyk_dir_path):
        os.makedirs(snyk_dir_path)
    ...
    original_json_path = os.path.join(DATA_DIR, "result",repo_name, module_name, 'original_version.json')
    ...
    # recompie and test
    compile_log_path = os.path.join(snyk_dir_path, 'compile_log.txt')
    test_log_path = os.path.join(snyk_dir_path, 'test_log.txt')
    ...
    tree_path = os.path.join(snyk_dir_path, 'verbose_tree.txt')
    json_path = tree_to_json(tree_path, os.path.join('/home1/kaixuan/ray/Dependabot_Snyk_dataset/Snyk', repo_name), module_name)
    if not json_path:
...
def recompile(repo_name, module_path):
    ...
    compile_log_path = os.path.join(DATA_DIR, 'snyk', repo_name, module_path, 'compile_log.txt')
    path_to_cloned_folder = os.path.join('/home1/kaixuan/ray/Dependabot_Snyk_dataset/Snyk', repo_name)
...
def test(repo_name, module_path):
    """test the module"""
    test_log_path = os.path.join(DATA_DIR, 'snyk', repo_name, module_path, 'test_log.txt')
    path_to_cloned_folder = os.path.join('/home1/kaixuan/ray/Dependabot_Snyk_dataset/Snyk', repo_name)
    path_to_project_folder = os.path.join('/home1/kaixuan/ray/Dependabot_Snyk_dataset/Snyk', repo_name)
...
def tree_to_json(tree_path:str, path_to_cloned_folder:str, relative_path_to_module:str):
    ...
    json_path = os.path.join(DATA_DIR, 'snyk', os.path.basename(path_to_cloned_folder), relative_path_to_module, 'current_version.json')
