...
def mvn_package(path_to_folder:str, relative_path_to_module:str):
    ...
    if relative_path_to_module == '.':
        command = f"cd {path_to_folder} && JAVA_HOME=/home/kaixuan/ray/jdk-17.0.12 mvn -Dmaven.test.skip=true -Dcheckstyle.skip=true clean package"
    else :
        command = f"cd {os.path.join(path_to_folder, relative_path_to_module)} && JAVA_HOME=/home/kaixuan/ray/jdk-17.0.12 mvn -Dmaven.test.skip=true -Dcheckstyle.skip=true clean package"
    ...
    if exit_status != 0 and relative_path_to_module != '.':
        ...
        command = f"cd {path_to_folder} && JAVA_HOME=/home/kaixuan/ray/jdk-17.0.12 mvn -Dmaven.test.skip=true -Dcheckstyle.skip=true -pl {relative_path_to_module} -am clean package "
        exit_status = os.system(command)
    if exit_status != 0:
...
def mvn_test(path_to_folder:str, relative_path_to_module:str):
    ...
    if relative_path_to_module == '.':
        command = f"cd {path_to_folder} && JAVA_HOME=/home/kaixuan/ray/jdk-17.0.12 mvn -Dcheckstyle.skip=true -Denforcer.skip=true -Dflatten.skip=true test"
    else :
        command = f"cd {os.path.join(path_to_folder, relative_path_to_module)} && JAVA_HOME=/home/kaixuan/ray/jdk-17.0.12 mvn -Dcheckstyle.skip=true \
    ...
    if exit_status != 0 and relative_path_to_module != '.':
        ...
        command = f"cd {path_to_folder} && JAVA_HOME=/home/kaixuan/ray/jdk-17.0.12 mvn -Dcheckstyle.skip=true -Denforcer.skip=true -Dflatten.skip=true -pl {relative_path_to_module} -am test"
        exit_status = os.system(command)
    if exit_status != 0:
...
def mvn_verbose_dependency_tree(path_to_folder:str, relative_path_to_module:str):
    ...
    command = f"cd {path_to_folder} && JAVA_HOME=/home/kaixuan/ray/jdk-17.0.12 mvn dependency:tree -pl {relative_path_to_module} -am -Dverbose -fae"
    print("generating dependency tree...")
    result = subprocess.run(command, shell=True, text=True, capture_output=True)
